#!/usr/bin/env python
"""
Test a holographic diffusion operator for the remaining hard31 galaxies.

Replace the current slope-driven L_eff adaptation by

    L_eff(r) = L_base * [1 + gamma_LH * eta_enclosed_bar(r) / (eta(r) + |deta/dr| r + eps)]

while keeping the current coupled eta-local trunk intact.
"""
...
OUT_PER = OUT_DIR / "hard31_holographic_diffusion_per_galaxy.csv"
OUT_SUM = OUT_DIR / "hard31_holographic_diffusion_summary.csv"
...
def build_diffusion_profile(rc_data, galaxy_name, trunk_params):
    ...
    eta_enclosed_bar = cumulative / np.arange(1, len(ev) + 1, dtype=float)
    denom = ev + np.abs(deta_dr) * rv + 1.0e-30
    correction = 1.0 + float(DEFAULT_COHERENCE_GAMMA) * (eta_enclosed_bar / denom)
    ...
    eta_full[valid] = np.interp(
        ...
        rv,
        eta_smooth,
        left=float(eta_smooth[0]),
        right=float(eta_smooth[-1]),
    )
    ...
    sigma_profile[valid] = np.interp(
        ...
        rv,
        sigma_valid,
        left=float(sigma_valid[0]),
        right=float(sigma_valid[-1]),
    )
    ...
    correction_full[valid] = np.interp(
        ...
        rv,
        correction,
        left=float(correction[0]),
        right=float(correction[-1]),
    )
    enclosed_full = np.full_like(r, np.nan, dtype=float)
    enclosed_full[valid] = np.interp(
        r[valid],
        rv,
        eta_enclosed_bar,
        left=float(eta_enclosed_bar[0]),
        right=float(eta_enclosed_bar[-1]),
    )
    ...
    profile["leff_diffusion_correction"] = correction_full
    profile["leff_diffusion_correction_median"] = float(np.nanmedian(correction_full[np.isfinite(correction_full)])) if np.any(np.isfinite(correction_full)) else 1.0
    profile["eta_enclosed_bar"] = enclosed_full
    profile["eta_enclosed_bar_median"] = float(np.nanmedian(enclosed_full[np.isfinite(enclosed_full)])) if np.any(np.isfinite(enclosed_full)) else np.nan
...
def main():
    ...
    for galaxy in hard31:
        ...
        profile = build_diffusion_profile(rc, galaxy, trunk_params)
        if profile is None:
            continue
        ...
        rows.append(
            {
                ...
                "leff_diffusion_correction_median": float(profile.get("leff_diffusion_correction_median", np.nan)),
        ...
        [
            {
                ...
                "mean_leff_diffusion_correction_median": float(np.nanmean(out["leff_diffusion_correction_median"])) if len(out) else np.nan,
                "median_leff_diffusion_correction_median": float(np.nanmedian(out["leff_diffusion_correction_median"])) if len(out) else np.nan,

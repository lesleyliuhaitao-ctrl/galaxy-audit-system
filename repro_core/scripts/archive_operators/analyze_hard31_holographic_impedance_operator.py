#!/usr/bin/env python
"""
Test a nonlinear holographic impedance operator on the final hard31 galaxies.

Operator:
    L_eff(r) = L_base * [1 + gamma_LH * (eta_enclosed_bar / eta(r))
                              * ln(1 + eta_base / eta(r))]
...
OUT_PER = OUT_DIR / "hard31_holographic_impedance_per_galaxy.csv"
OUT_SUM = OUT_DIR / "hard31_holographic_impedance_summary.csv"
...
def build_impedance_profile(rc_data, galaxy_name, trunk_params):
    ...
    eps = 1.0e-30
    correction = 1.0 + float(DEFAULT_COHERENCE_GAMMA) * (
        (eta_enclosed_bar / np.maximum(ev, eps))
        * np.log(1.0 + float(eta_base) / np.maximum(ev, eps))
    )
    ...
    enclosed_full[valid] = np.interp(
        r[valid], rv, eta_enclosed_bar, left=float(eta_enclosed_bar[0]), right=float(eta_enclosed_bar[-1])
    )
    ...
    profile["leff_impedance_correction"] = correction_full
    profile["leff_impedance_correction_median"] = float(np.nanmedian(correction_full[np.isfinite(correction_full)])) if np.any(np.isfinite(correction_full)) else 1.0
    ...
    profile["eta_crit"] = float(eta_base)
        ...
        [
            {
                ...
                "mean_leff_impedance_correction_median": float(np.nanmean(out["leff_impedance_correction_median"])) if len(out) else np.nan,
                "median_leff_impedance_correction_median": float(np.nanmedian(out["leff_impedance_correction_median"])) if len(out) else np.nan,
